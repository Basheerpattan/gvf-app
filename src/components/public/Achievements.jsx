...
export function Achievements() {
  ...
  useEffect(() => {
    supabase.from('achievements').select('*').order('display_order')
      .then(({ data }) => setAchievements(data?.length ? data : defaultAchievements))
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="inline-block text-emerald-400 font-semibold text-sm tracking-widest uppercase mb-4">
            {settings.achievements_badge}
          </span>
          <h2 className="font-display font-bold text-3xl sm:text-4xl text-white mb-4">
            {settings.achievements_title}
          </h2>
          <p className="text-emerald-200/70 max-w-xl mx-auto">{settings.achievements_desc}</p>
        </div>
